-- Create the database if it doesn't already exist and set it as the active database.
CREATE DATABASE IF NOT EXISTS foodlink;
USE foodlink;

--
-- Table structure for table `users`
-- Added 'username' for easier login, 'photo_url' for profiles, and 'is_verified' for admin approval.
--
...
-- Added 'photo_url' to show images of the food and added `updated_at` for tracking changes.
--
...
-- This table tracks the positive impact of each completed donation.
--
...
-- Using clear, hashed passwords for 'password123' and more descriptive examples.
--
INSERT INTO `users` (`username`, `email`, `password`, `name`, `type`, `address`, `phone`, `is_verified`) VALUES
...
INSERT INTO `donations` (`food_name`, `quantity`, `description`, `expiry_time`, `donor_id`, `ngo_id`, `location`, `status`, `claimed_at`) VALUES
...
('Assorted Bakery Goods', '5 kg', 'Includes bread, croissants, and muffins from the morning batch. Best consumed today.', NOW() + INTERVAL 12 HOUR, 2, NULL, 'City Bakery, 456 Oak Ave', 'available'),
...
('Fresh Vegetable Boxes', '25 boxes', 'Slightly imperfect but fresh vegetables including tomatoes, onions, and potatoes.', NOW() + INTERVAL 2 DAY, 2, NULL, 'City Bakery, 456 Oak Ave', 'available');

INSERT INTO `impact` (`donation_id`, `meals_provided`, `weight_kg`, `people_served`, `notes`) VALUES
